...
// check day: only Mon-Fri
const dow = manilaNow().day(); // 0 Sun .. 6 Sat
if (dow === 0 || dow === 6) { req.flash('error','Bets allowed Monday to Friday only'); return res.redirect('/bet'); }
...
// validate numbers 1..50 and unique
const valid = nums.every(n=>n>=1 && n<=50) && (new Set(nums)).size === 6;
if (!valid) { req.flash('error','Numbers must be 6 unique values between 1 and 50'); return res.redirect('/bet'); }
const price = 20;
// check balance
db.get('SELECT balance FROM users WHERE id = ?', [req.user.id], (err, row)=>{
if (err) { req.flash('error','DB error'); return res.redirect('/bet'); }
const balance = row.balance || 0;
if (balance < price) { req.flash('error','Insufficient balance. Please top-up'); return res.redirect('/topup'); }
// deduct and record bet
const now = new Date().toISOString();
db.run('UPDATE users SET balance = balance - ? WHERE id = ?', [price, req.user.id], function(err2){
db.run('INSERT INTO bets (user_id,numbers,amount,status,created_at) VALUES (?,?,?,?,?)', [req.user.id, nums.join(','), price, 'pending', now], function(err3){
req.flash('success','Bet placed successfully. Good luck!');
res.redirect('/dashboard');
});
});
});
...
// Withdraw request
app.get('/withdraw', ensureAuthenticated, (req, res) => res.render('withdraw', { user: req.user, messages: req.flash() }));
...
db.get('SELECT balance FROM users WHERE id = ?', [req.user.id], (err, row)=>{
const balance = row.balance || 0;
if (balance < a) { req.flash('error','Insufficient balance'); return res.redirect('/withdraw'); }
const id = uuidv4();
db.run('INSERT INTO transactions (id,user_id,type,amount,status,reference,created_at) VALUES (?,?,?,?,?,?,?)', [id, req.user.id, 'withdraw', a, 'pending', gcash_number || '', new Date().toISOString()], function(err2){
// mark amount as reserved by subtracting balance
db.run('UPDATE users SET balance = balance - ? WHERE id = ?', [a, req.user.id], function(err3){
req.flash('success','Withdraw request created. Admin will process it.');
res.redirect('/dashboard');
});
});
});
